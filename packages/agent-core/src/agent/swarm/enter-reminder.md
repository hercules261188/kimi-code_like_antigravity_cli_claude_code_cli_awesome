...
- Unless the user explicitly specifies a limit, do not try to conserve the number of agents. Decompose work as finely as possible while keeping subagent responsibilities non-conflicting; combine tasks only when they are genuinely inseparable. If the subagents only need to read, inspect, or report back without making changes, their scopes may overlap slightly when that improves coverage or cross-checking.
