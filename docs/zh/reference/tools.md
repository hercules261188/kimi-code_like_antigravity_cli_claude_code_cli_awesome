...
| `AgentSwarm` | 自动放行 | 从提示词模板启动多个子 Agent |
...
**`AgentSwarm`** 从共享的 `prompt_template` 和 `items` 数组启动多个子 Agent。模板必须包含 `{{item}}` 占位符；每个 item 会替换该占位符，并启动一个子 Agent。传入 `subagent_type` 可以指定整个 swarm 中所有子 Agent 使用的 profile；省略时默认使用 `coder`。本工具接受 2 到 50 个 item，会等待全部子 Agent 完成，并返回聚合报告。在 TUI 中，前台 swarm 会在输入框上方显示实时 `Agent swarm` 进度面板。
